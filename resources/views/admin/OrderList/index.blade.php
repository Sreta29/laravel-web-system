        ...
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center">
                            ...
                            <div class="relative w-full">
                                ...
                                <form action="{{ route('configuration.orderlist.index') }}" method="GET">
                                <input type="text" name="search" value="{{$search}}" onkeydown="if (event.key === 'Enter') this.form.submit()" id="simple-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search" required="">
                                </form>
                            </div>
                        </form>
                    </div>
                    ...
                    @php($user = Auth::user()) 
                    @if($user->role == 'System Admin' || $user->role == 'User')
                    <a href="{{ route('configuration.orderlist.create') }}">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Zone</th>
                                <th scope="col" class="px-4 py-3">Email</th>
                                <th scope="col" class="px-4 py-3">Collection Date</th>
                                <th scope="col" class="px-4 py-3">Collector</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">{{ $orderlist->orderid }}</td>
                                <td class="px-4 py-3">{{ $orderlist->orderdate }}</td>
                                <td class="px-4 py-3">{{ $orderlist->zone }}</td>
                                <td class="px-4 py-3">{{ $orderlist->email }}</td>
                                <td class="px-4 py-3">{{ $orderlist->collectdate }}</td>
                                <td class="px-4 py-3">{{ $orderlist->collector }}</td>
                                <td class="px-4 py-3">{{ $orderlist->status}} </td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div class="flex items-center space-x-4">
                                        @if($user->role == 'System Admin')
                                            <a href="{{ route('configuration.orderlist.edit', $orderlist->id) }}">
                                                <button type="button" class="py-2 px-3 flex items-center text-sm font-medium text-center text-white text-blue-700 bg-primary-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                            ...
                                            <form method="POST" action="{{ route('configuration.orderlist.delete', $orderlist->id) }}" onsubmit="return confirm('Are you sure you want to delete this item?')">
                                                @csrf
                                                @method('DELETE')
                                                <button class="flex items-center text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 
                                                focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 
                                                py-2 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                        <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 
                                                        100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                                                    </svg>
                                                    Delete
                                                </button>
                                            </form>
                                        ...
                                        @if( $user->role == 'User')
                                            ...
                                            <a href="{{ route('configuration.orderlist.edit', $orderlist->id) }}">
                                                <button type="button" class="py-2 px-3 flex items-center text-sm font-medium text-center text-blue-700 bg-primary-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                        <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                                        <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                                                    </svg>
                                            ...
                                            <a href="{{ route('pay.index', $orderlist->id) }}">
                                                <button type="button" class="py-2 px-3 flex items-center text-sm font-medium text-center text-white text-blue-700 bg-primary-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
@endsection <!-- End the content section --><!-- Start block -->
